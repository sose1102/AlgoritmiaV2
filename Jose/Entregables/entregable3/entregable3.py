        ...
        def successors(self) -> Iterable["ScoredDecisionSequence"]:
            if not self.is_solution():
                ...
                for pM in posiblesMovimientos:
                    ...
                    if board.has_tile(copia.b1) and board.has_tile(
                            copia.b2):  # En caso de que la copia salga del tablero no la coge
                        yield self.add_decision(pM,
                                                Extra(copia))  # Añadir las decisiones que se puedan tomar en cada caso
